...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
        <h2>Mejor Indie</h2>
        <p><input type="radio" name="indie" value = "Cult of the Lamb"> Cult of the Lamb </p>
        <p><input type="radio" name="indie" value = "Neon White"> Neon White</p>
        <p><input type="radio" name="indie" value = "Sifu"> Sifu </p>
        <p><input type="radio" name="indie" value = "Stray"> Stray</p>
        <p><input type="radio" name="indie" value = "TUNIC"> TUNIC </p>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($_REQUEST['indie'] !="") {
                ...
                if($id_us !="" && $id_juego !="") { //se comprueba que los dos id no estan vacios
                   try {
                        $sqlVot= $cons->query("SELECT id_votacion FROM votaciones WHERE categoria='mejor juego indie' and id_usuario=$id_us"); //se comprueba si el usuario ya ha votado en sa categoria
                    ...
                    if($id_votacion == "" ){
                        try {
                            $sql= $cons->query("INSERT INTO votaciones (id_juego, id_usuario, categoria) VALUES ('$id_juego' , '$id_us', 'mejor juego indie')"); //se añade la votacion a la tabla
                        } catch (Exception $e) {
                            echo "Se ha producido un error : " . $e->getMessage();
                        }
