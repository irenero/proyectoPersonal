        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($_REQUEST['anio'] !="") {
                ...
                if($id_us !="" && $id_juego !="") { //se comprueba que los dos id no estan vacios
                   try {
                        $sqlVot= $cons->query("SELECT id_votacion FROM votaciones WHERE categoria='Juego del anio' and id_usuario=$id_us"); //se comprueba si el usuario ya ha votado en sa categoria
                    ...
                    if($id_votacion == "" ){
                        try {
                            $sql= $cons->query("INSERT INTO votaciones (id_juego, id_usuario, categoria) VALUES ('$id_juego' , '$id_us', 'Juego del anio')"); //se añade la votacion a la tabla
                        } catch (Exception $e) {
                            echo "Se ha producido un error : " . $e->getMessage();
                        }
