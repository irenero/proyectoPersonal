...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
        ...
        <p><input type="radio" name="virtual" value = "After the Fall"> After the Fall </p>
        <p><input type="radio" name="virtual" value = "Among Us VR"> Among Us VR  </p>
        <p><input type="radio" name="virtual" value = "BONELAB"> BONELAB</p>
        <p><input type="radio" name="virtual" value = "Moss: Book II"> Moss: Book II </p>
        <p><input type="radio" name="virtual" value = "Red Matter 2"> TRed Matter 2 </p>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($_REQUEST['virtual'] !="") {
                ...
                if($id_us !="" && $id_juego !="") { //se comprueba que los dos id no estan vacios
                   try {
                        $sqlVot= $cons->query("SELECT id_votacion FROM votaciones WHERE categoria='mejor juego de realidad virtual' and id_usuario=$id_us"); //se comprueba si el usuario ya ha votado en sa categoria
                    ...
                    if($id_votacion == "" ){
                        try {
                            $sql= $cons->query("INSERT INTO votaciones (id_juego, id_usuario, categoria) VALUES ('$id_juego' , '$id_us', 'mejor juego de realidad virtual')"); //se añade la votacion a la tabla
                        } catch (Exception $e) {
                            echo "Se ha producido un error : " . $e->getMessage();
                        }
