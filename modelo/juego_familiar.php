...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
        ...
        <p><input type="radio" name="familiar" value = "DNF Duel"> DNF Duel </p>
        <p><input type="radio" name="familiar" value = "LEGO Star Wars: The Skywalker Saga">LEGO Star Wars: The Skywalker Saga</p>
        <p><input type="radio" name="familiar" value = "Mario + Rabbids Sparks of Hope"> Mario + Rabbids Sparks of Hope</p>
        <p><input type="radio" name="familiar" value = "Nintendo Switch Sports"> Nintendo Switch Sports</p>
        <p><input type="radio" name="familiar" value = "Splatoon 3"> Splatoon 3 </p>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($_REQUEST['familiar'] !="") {
                ...
                if($id_us !="" && $id_juego !="") { //se comprueba que los dos id no estan vacios
                   try {
                        $sqlVot= $cons->query("SELECT id_votacion FROM votaciones WHERE categoria='mejor juego familiar' and id_usuario=$id_us"); //se comprueba si el usuario ya ha votado en sa categoria
                    ...
                    if($id_votacion == "" ){
                        try {
                            $sql= $cons->query("INSERT INTO votaciones (id_juego, id_usuario, categoria) VALUES ('$id_juego' , '$id_us', 'mejor juego familiar')"); //se añade la votacion a la tabla
                        } catch (Exception $e) {
                            echo "Se ha producido un error : " . $e->getMessage();
                        }
