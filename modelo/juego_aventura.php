...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
        ...
        <p><input type="radio" name="aventura" value = "A Plague Tale: Requiem"> A Plague Tale: Requiem </p>
        <p><input type="radio" name="aventura" value = "God of War: Ragnarok"> God of War: Ragnarok</p>
        <p><input type="radio" name="aventura" value = "Horizon Forbidden West"> Horizon Forbidden West </p>
        <p><input type="radio" name="aventura" value = "Stray"> Stray </p>
        <p><input type="radio" name="aventura" value = "TUNIC"> TUNIC </p>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($_REQUEST['aventura'] !="") {
                ...
                if($id_us !="" && $id_juego !="") { //se comprueba que los dos id no estan vacios
                   try {
                        $sqlVot= $cons->query("SELECT id_votacion FROM votaciones WHERE categoria='mejor juego de aventura' and id_usuario=$id_us"); //se comprueba si el usuario ya ha votado en sa categoria
                    ...
                    if($id_votacion == "" ){
                        try {
                            $sql= $cons->query("INSERT INTO votaciones (id_juego, id_usuario, categoria) VALUES ('$id_juego' , '$id_us', 'mejor juego de aventura')"); //se añade la votacion a la tabla
                        } catch (Exception $e) {
                            echo "Se ha producido un error : " . $e->getMessage();
                        }
