...
<body>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
        ...
        <p><input type="radio" name="rol" value = "Elden Ring"> Elden Ring</p>
        <p><input type="radio" name="rol" value = "Live a Live">Live a Live</p>
        <p><input type="radio" name="rol" value = "Pokemon Legends: Arceus"> Pokémon Legends: Arceus </p>
        <p><input type="radio" name="rol" value = "Triangle Strategy"> Triangle Strategy </p>
        <p><input type="radio" name="rol" value = "Xenoblade Chronicles 3"> Xenoblade Chronicles 3</p>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if($_REQUEST['rol'] !="") {
                ...
                if($id_us !="" && $id_juego !="") { //se comprueba que los dos id no estan vacios
                   try {
                        $sqlVot= $cons->query("SELECT id_votacion FROM votaciones WHERE categoria='mejor juego de rol' and id_usuario=$id_us"); //se comprueba si el usuario ya ha votado en sa categoria
                    ...
                    if($id_votacion == "" ){
                        try {
                            $sql= $cons->query("INSERT INTO votaciones (id_juego, id_usuario, categoria) VALUES ('$id_juego' , '$id_us', 'mejor juego de rol')"); //se añade la votacion a la tabla
                        } catch (Exception $e) {
                            echo "Se ha producido un error : " . $e->getMessage();
                        }
